defmodule Customer.Web.Api.V1.FavoriteJobController do
  ...
  def index(conn, _params, current_user, _claims) do
    favorite_jobs = Query.FavoriteJob.all_by(%{user_id: current_user.id})
    render(conn, "index.json", %{favorite_jobs: favorite_jobs})
  end

  def show(conn, %{"id" => job_id}, current_user, _claims) do
    with {:ok, favorite_job} <- Query.FavoriteJob.get_by(%{user_id: current_user.id, job_id: job_id}) do
      render(conn, "show.json", %{favorite_job_id: favorite_job.id})
    end
  end

  def create(conn, %{"id" => job_id}, current_user, _claims) do
    with {:ok, _} <- Command.FavoriteJob.favorite(favorite_job_params(current_user.id, job_id)) do
      send_resp(conn, 201, "")
    end
  end

  def update(conn, %{"id" => job_id} = params, current_user, _claims) do
    with {:ok, _ } <- Command.FavoriteJob.update(%{user_id: current_user.id, job_id: job_id}, Converter.convert_key_to_atom(Map.delete(params, "id"))) do
      send_resp(conn, 200, "")
    end
  end

  def delete(conn, %{"id" => job_id}, current_user, _claims) do
    with {:ok, _} <- Command.FavoriteJob.unfavorite(%{user_id: current_user.id, job_id: job_id}) do
      send_resp(conn, 200, "")
    end
  end
